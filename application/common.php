...
function get_category_children($cid)
{
    ...
    $children = Db::name('category')->where(['path' => ['like', "%,{$cid},%"]])->order('sort', 'asc')->select();
...
function get_articles_by_cid($cid, $limit = 10, $where = [], $order = [], $filed = [])
{
    ...
    $fileds = array_merge(['id', 'cid', 'title', 'tags', 'introduction', 'thumb', 'reading', 'publish_time'], (array)$filed);
 ...
 * @return bool|false|PDOStatement|string|\think\Collection
 */
function get_college_articles_by_cid($cid, $limit = 10, $where = [], $order = [], $filed = [])
{
    if (empty($cid)) {
        return false;
    }
    ...
    $fileds = array_merge(['id', 'cid', 'title', 'gradation', 'code', 'local_code', 'introduction', 'thumb', 'reading', 'publish_time'], (array)$filed);
    $map    = array_merge(['cid' => ['IN', $ids], 'status' => 1, 'publish_time' => ['<= time', date('Y-m-d H:i:s')]], (array)$where);
    $sort   = array_merge(['is_top' => 'DESC', 'sort' => 'DESC', 'publish_time' => 'DESC'], (array)$order);

    $article_list = Db::name('college_article')->where($map)->field($fileds)->order($sort)->limit($limit)->select();
...
function get_articles_by_cid_paged($cid, $page_size = 15, $where = [], $order = [], $filed = [])
{
    ...
    $article_list = Db::name('article')->where($map)->field($fileds)->order($sort)->paginate(10);
